...
func (a *stubAgents) promptFor(name string) (uuid.UUID, string, bool) {
	...
	for _, agent := range a.spawned {
		if strings.Contains(agent.prompt, "# Step: "+name+"\n") {
...
func (c *stubUserConfig) AgentDefault(enums.Effort) (*output_itf.AgentDefault, error) {
...
func e2eWorkflowConfig() *input_itf.WorkflowConfig {
...
func newHarness(t *testing.T) *harness {
	...
	agents := &stubAgents{}
	workflows.TrackLiveAgents(agents)
	...
	runner, err := coordinator.InitV1(e2eWorkflowConfig(), workflows, agents, history, &stubLogger{})
	if err != nil {
		t.Fatalf("init coordinator: %v", err)
	}
	t.Cleanup(runner.Stop)

	roles, err := role_manager.InitV1(store.RoleStore(), role_archive.InitV1())
	if err != nil {
		t.Fatalf("init role manager: %v", err)
	}

	control, err := workflow_control.InitV1(
		e2eControlConfig(),
		workflows,
		runner,
		roles,
...
func stepStateIn(state map[string]any, name string) map[string]any {
...
func TestAControlCallerRunsAGatedGraphEndToEnd(t *testing.T) {
	...
	created := h.callControl(t, "create_workflow", map[string]any{
		"project_dir_path": h.projectDir,
		"autostart":        true,
		"steps": []map[string]any{
			{
				...
				"inputs":           map[string]string{"area": "interface/core"},
	...
	if created["started"] != true {
		t.Fatalf("create_workflow did not start the run: %v", created)
	}

	workflowID, _ := created["workflow_id"].(string)
	if workflowID == "" {
		t.Fatalf("create_workflow returned no workflow id: %v", created)
	}

	stepIDs, _ := created["step_ids"].(map[string]any)
	if len(stepIDs) != 2 {
		t.Fatalf("create_workflow mapped %d client ids, want 2", len(stepIDs))
	}

	var surveyAgent uuid.UUID
	var surveyPrompt string

	waitUntil(t, "the survey step to reach an agent", func() bool {
	...
	if !strings.Contains(surveyPrompt, "List every port in interface/core.") {
		t.Fatalf("the input was not rendered into the prompt:\n%s", surveyPrompt)
	}

	if _, _, found := h.agents.promptFor("wire the port"); found {
		t.Fatal("the dependent step ran before its dependency finished")
	}

	h.callHarness(t, surveyAgent, "report_step", map[string]any{
	...
	waitUntil(t, "the gate to open", func() bool {
		state := h.callControl(t, "workflow_status", map[string]any{"workflow_id": workflowID})
		step := stepStateIn(state, "survey the ports")

		return step != nil && step["status"] == string(enums.StepAwaitingReview)
	})

	if _, _, found := h.agents.promptFor("wire the port"); found {
		t.Fatal("the dependent step ran while the gate was still closed")
	}

	gated := h.callControl(t, "workflow_status", map[string]any{"workflow_id": workflowID})
	surveyStep := stepStateIn(gated, "survey the ports")

	if surveyStep["tldr"] != "Listed the doors this program opens to the outside world." {
		t.Fatalf("the tldr did not come back through workflow_status: %v", surveyStep)
	}

	h.callControl(t, "answer_review", map[string]any{
		"step_id":  stepIDs["survey"],
		"accepted": true,
	})

	var wirePrompt string

	waitUntil(t, "the dependent step to be released", func() bool {
	...
	if !strings.Contains(wirePrompt, "seven ports, each with the call that reaches it") {
		t.Fatalf("the handoff did not reach the dependent step:\n%s", wirePrompt)
	}

	listed := h.callControl(t, "list_workflows", map[string]any{})

	text, isText := listed["text"].(string)
	if !isText || !strings.Contains(text, workflowID) {
		t.Fatalf("list_workflows did not carry the workflow back: %v", listed)
	}

	h.callControl(t, "cancel_workflow", map[string]any{"workflow_id": workflowID})

	waitUntil(t, "the workflow to report itself cancelled", func() bool {
		state := h.callControl(t, "workflow_status", map[string]any{"workflow_id": workflowID})
		step := stepStateIn(state, "wire the port")

		return step != nil && step["status"] == string(enums.StepCancelled)
	})
}
...
func TestTheStdioShimCarriesAControlCallIntoTheApp(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(replies[1]), &created); err != nil {
		t.Fatalf("parse create_workflow reply: %v", err)
	}

	if created.Result.IsError || len(created.Result.Content) == 0 {
		t.Fatalf("create_workflow through the shim failed: %s", replies[1])
	}

	if !strings.Contains(created.Result.Content[0].Text, `"workflow_id"`) {
		t.Fatalf("create_workflow through the shim returned no workflow: %s", created.Result.Content[0].Text)
	}
}
