/**
 * A diff is read when a step is opened, not carried on its report, so the mock
 * keeps the changes here rather than in the workflow fixtures. A step the mock
 * workflows name gets the diff written for it; every other step id — the ones a
 * simulated run invents — gets a deterministic slice of the same changes, the
 * way mockOutcome derives a run from a step id.
 */
...
-Forwarded calls inherit the agent's lifetime, so a hung server hangs the step.
+Forwarded calls carry a deadline from \`mcp_proxy.forward_deadline\`, so a hung
+server fails one call instead of stalling the step that made it.`,
    },
    walReplay: {
...
+}`,
    ...
    workflowRename: {
        path: 'internal/implementation/workflow_manager/v1.go',
        oldPath: 'internal/implementation/step_manager/v1.go',
        changeType: 'renamed',
        additions: 5,
        deletions: 5,
        unifiedDiff: `--- a/internal/implementation/step_manager/v1.go
+++ b/internal/implementation/workflow_manager/v1.go
@@ -1,10 +1,10 @@
...
-type stepManagerV1 struct {
+type workflowManagerV1 struct {
     retry RetryPolicy
 }`,
    },
    retryTable: {
        path: 'internal/implementation/workflow_manager/retry.go',
        oldPath: '',
        changeType: 'created',
        additions: 18,
        deletions: 0,
        unifiedDiff: `--- /dev/null
+++ b/internal/implementation/workflow_manager/retry.go
@@ -0,0 +1,18 @@
+package workflow_manager
+
+import "time"
...
+}
+
+func (s *workflowManagerV1) waitBefore(attempt int) time.Duration {
...
const DIFF_BY_STEP: Record<string, FileChange[]> = {
    'step-6': [],
    'step-7': [CHANGES.proxyDeadline, CHANGES.proxyConfig, CHANGES.proxyYaml],
    'step-8': [CHANGES.proxyDocs],
    'step-9': [CHANGES.walProbe, CHANGES.walReplay],
    'step-10': [CHANGES.walProbe],
    'step-12': [CHANGES.workflowRename],
    'step-16': [],
    'step-19': [CHANGES.proxyDeadline, CHANGES.proxyConfig],
}
...
export function mockStepDiff(stepId: string): FileChange[] {
    const named = DIFF_BY_STEP[stepId]
    if (named) return structuredClone(named)

    const seed = seedOf(stepId)
